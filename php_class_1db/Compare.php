...
class db_compare {
    ...
    private function compare() {
         ...
         */
        $this->get_indices();
        $results = $this->manage_indices($this->DB2['indices'], $this->index1);
        $sql_commands_to_run = array_merge($sql_commands_to_run, $results);
     ...
     * @see https://stackoverflow.com/questions/6914105/2d-multidimensional-array-to-1d-array-in-php
     ...
     */
    private function compare_table_structures($live_tables, $development_tables) {
        ...
        foreach ($development_tables as $table) {
            $development_table_structures[$table] = $this->DB2['create'][$table][0];
        }
        /*
         * generate the sql for each table in the live database
         */
        foreach ($live_tables as $table) {
            $query = $this->DB1->query("SHOW CREATE TABLE `$table` -- live");
            $table_structure = $this->array_2d_to_1d(mysqli_fetch_all($query, MYSQLI_NUM));
            array_shift($table_structure);
                    ...
                    foreach ($fields as $field) {
                        if ($this->in_array_recursive($field['name'], $destination_field_structures[$table])) {
                            ...
                            */
                            ...
                            $add_field .= (isset($field['IS_NULLABLE']) && $field['IS_NULLABLE'] != 'NO') ? ' NULL ' : ' NOT NULL ';
    ...
    public function create_db_snapshot() {
        $db = array();
        $db = $this->table_field_data($this->DB1, '', 1);
        $this->get_indices();
        ...
        foreach (array_keys($db) as $index) {
            if ($index != 'indices' && $index != 'create') {
                $query = $this->DB1->query("SHOW CREATE TABLE `$index` -- dev");
                $table_structure = $this->array_2d_to_1d(mysqli_fetch_all($query, MYSQLI_NUM));
                array_shift($table_structure);
                $db['create'][$index] = $table_structure;
            }
        }
