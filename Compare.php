...
class Compare extends MX_Controller
{
    ...
    function index()
    {
        ...
        $sql_commands_to_run = (is_array($tables_to_update) && !empty($tables_to_update)) ? array_merge($sql_commands_to_run, $this->update_existing_tables($tables_to_update)) : '';
				...
				array_shift($sql_commands_to_run); // fixes error where tables to add and drop aren't shown
	        ...
	        foreach ($source_field_structures as $table => $fields)
	        {
	            foreach ($fields as $field)
	            {
	                ...
	                else
	                {
	                    ...
	                    $add_field .= (isset($fields[$n]["default"]) && $fields[$n]["default"] != '') ? ' DEFAULT \'' . $fields[$n]["default"] . '\'' : '';
	                    $add_field .= " DEFAULT " . $field["default"];
	                    $add_field .= (isset($fields[$n]["max-length"]) && $fields[$n]["max-length"] != '') ? ' ' . $fields[$n]["max-legth"] : '';
	                    $add_field .= ';';
