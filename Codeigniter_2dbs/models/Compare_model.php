...
class Compare_model extends CI_Model {
    ...
    public function manage_indices() {
        ...
        foreach ($this->index2 as $index) {
            if (!(array_key_exists($index['table'] . '-' . $index['index'], $this->index1))) {
    ...
    public function table_list($db) {
        if ($db == 1) {
            return $this->DB1->list_tables();
        } else {
            return $this->DB2->list_tables();
        }
    }
}
